...
import { columns, type Data } from "../types/types";
import { ButtonMain } from "../../components/ButtonNovo";
...
export const Home = () => {
    ...
    const handleLoadingDatas = async () => {
        ...
        setLoading(true)
        setDatas(response.data.content.vehicles);
    };
    ...
    return (
        <div className="bg-customBackground g-10">
            <Navbar />
            <ButtonMain/>
            <CardMain>
                <MapMain datasCar={datas} />
            </CardMain>
            {loading ? (
                <TableMain data={datas} columns={columns} />
            ):(
